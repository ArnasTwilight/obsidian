...
На текущий момент проблема решена установкой параметра php:
```php
always_populate_raw_post_data=-1
```
Поскольку этот параметр управляет **`$HTTP_RAW_POST_DATA`**, начиная с **PHP 7.0**, этот параметр **удалён** (так как `$HTTP_RAW_POST_DATA` устарел в пользу `php://input`), но в ранних версиях (PHP 5.6 и ниже) его можно было настраивать. Судя по всему ББД использовал **`$HTTP_RAW_POST_DATA`**, потому и падала ошибка при запросе с типом POST.

Если по какой-то причине до сих пор валится ошибка при попытке обратиться к ендпоинтам через метод POST, то достаточно просто перезапустить проект:
